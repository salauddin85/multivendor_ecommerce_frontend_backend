...
const vendorSchema = Yup.object().shape({
  ...
  nid_card_pic: Yup.mixed().required("NID card image is required"),
...
export function VendorForm() {
  ...
  const handleSubmit = async (values: any, { setFieldError }: any) => {
    ...
    // console.log("befor submit data",data)
    data.append("email", values.email);
    data.append("password", values.password);
    data.append("store_name", values.store_name);
    data.append("product_details", values.product_details);
    data.append("address", values.address);
    data.append("product_image", values.product_image);
    data.append("nid_card_pic", values.nid_card_pic);
    data.append("trade_license", values.trade_license);
    data.append("phone_number", values.phone_number);
    data.append("first_name", values.first_name);
    data.append("last_name", values.last_name);
    // console.log("data", data);
    try {
      const response = await registerUser("vendor", data);
      if (response.success) {
        toast.success(
          "Vendor registration successful! Redirecting to login..."
        );
        setTimeout(() => {
  ...
  return (
    <Formik
      initialValues={{
        ...
        product_details: store.formData.product_details || "",
        product_image: null,
        nid_card_pic: null,
        trade_license: null,
      ...
      {({ errors, touched, values, setFieldValue }) => (
        <Form className="space-y-4">
          ...
          <div className="form-group">
            ...
            />
            {errors.product_details && touched.product_details && (
              <p className="form-error">{errors.product_details}</p>
            )}
          </div>
          ...
          <div className="form-group">
            ...
            {values.product_image && (
              <p className="text-sm text-muted-foreground mt-2">
                Selected: {(values.product_image as any).name}
              </p>
            )}
            {errors.product_image && touched.product_image && (
              <p className="form-error">{errors.product_image}</p>
            )}
          </div>
          ...
          <div className="form-group">
            ...
            <input
              ...
              hidden
              onChange={(e) =>
                setFieldValue("nid_card_pic", e.target.files?.[0])
              }
            />
            {values.nid_card_pic && (
              <p className="text-sm text-muted-foreground mt-2">
                Selected: {(values.nid_card_pic as any).name}
              </p>
            )}
            {errors.nid_card_pic && touched.nid_card_pic && (
              <p className="form-error">{errors.nid_card_pic}</p>
            )}
          </div>
