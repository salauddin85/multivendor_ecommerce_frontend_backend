  ...
  return (
    <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl border shadow-sm p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button className="px-5 py-2 rounded-md text-white bg-orange-400 hover:bg-orange-500 transition">
          Update Profile
        </button>
      </div>
      ...
      <SectionCard title="Basic Information">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <InfoItem label="Account Status" value={status} badge />
        </div>
      </SectionCard>
      ...
      <SectionCard title="Product Information">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <InfoItem label="Product Details" value={product_details} />
          <ImageItem
            label="Product Image"
            src={`${process.env.NEXT_PUBLIC_BACKEND_API_URL}${product_image}`}
          />
        </div>
      </SectionCard>
      ...
      <SectionCard title="Verification Documents">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <ImageItem
            label="NID Card"
            src={`${process.env.NEXT_PUBLIC_BACKEND_API_URL}${nid_card_pic}`}
          />
          <ImageItem
            label="Trade License"
