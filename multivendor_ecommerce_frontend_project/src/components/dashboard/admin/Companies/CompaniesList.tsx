...
interface CompaniesListProps {
  companies_list: ApiResponse;
}

export default function CompaniesList({ companies_list }: CompaniesListProps) {
  const { data: companies, pagination } = companies_list;
  ...
  const calculateStatistics = (companies: StoreOwner[]) => {
    const statusCount = {
      pending: 0,
      approved: 0,
      rejected: 0,
    ...
    return {
      ...
      activeUsers: companies.filter((company) => company.user.is_active).length,
    };
  };

  const statistics = calculateStatistics(filteredCompanies);
  ...
  const getFullName = (company: StoreOwner) => {
    return `${company.user.first_name} ${company.user.last_name}`.trim();
  };
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (company) =>
          getFullName(company).toLowerCase().includes(term) ||
          company.user.email.toLowerCase().includes(term) ||
          company.phone_number.includes(term) ||
          company.address.toLowerCase().includes(term) ||
          company.store_details.toLowerCase().includes(term),
      );
    }
  ...
  }, [searchTerm, statusFilter, sortConfig, companies]);
  ...
  const handleBulkAction = (action: string) => {
    ...
    console.log(`Bulk ${action} for companies:`, selectedCompanies);

    // Show confirmation dialog
    if (
      window.confirm(
        `Are you sure you want to ${action} ${selectedCompanies.length} company/companies?`,
      )
    ) {
      // Perform action
      alert(`Performing ${action} on selected companies...`);
      setSelectedCompanies([]);
    }
  };
    ...
    <div className="bg-white rounded-xl shadow-lg p-4 md:p-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Manage store owners and company registrations
            </p>
          </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="relative">
              <input
                type="text"
                placeholder="Search by name, email, address, or store details..."
                value={searchTerm}
            ...
            <div>
              ...
              <p className="text-xs text-yellow-600 mt-1">
                {statistics.totalCompanies > 0
                  ? `${Math.round((statistics.pending / statistics.totalCompanies) * 100)}%`
                  : "0%"}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-green-600 mt-1">
                {statistics.totalCompanies > 0
                  ? `${Math.round((statistics.approved / statistics.totalCompanies) * 100)}%`
                  : "0%"}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-red-600 mt-1">
                {statistics.totalCompanies > 0
                  ? `${Math.round((statistics.rejected / statistics.totalCompanies) * 100)}%`
                  : "0%"}
              </p>
            </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredCompanies.length > 0 ? (
                  filteredCompanies.map((company) => (
                    <tr
                      key={company.id}
                      className={`hover:bg-gray-50 transition-colors duration-150 ${selectedCompanies.includes(company.id) ? "bg-blue-50" : ""}`}
                    >
                        ...
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            ...
                            <div className="text-xs text-gray-500 mt-1">
                              ID: #{company.id} • Joined:{" "}
                              {formatDate(company.user.date_joined)}
                            </div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col space-y-2">
                          ...
                          <div>
                            {getUserStatusBadge(company.user.is_active)}
                          </div>
                          <div className="text-xs text-gray-500">
                            Store Details: {company.store_details}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col space-y-2">
                          {/* NID Card Link */}
                          <a
                            href={`${process.env.NEXT_PUBLIC_BACKEND_API_URL}${company.nid_card_image}`}
                            target="_blank"
                          ...
                          {/* Trade License Link */}
                          <a
                            href={`${process.env.NEXT_PUBLIC_BACKEND_API_URL}${company.trade_license}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center text-sm text-blue-600 hover:text-blue-800"
                          >
      ...
      <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-sm text-gray-600">
        <div>
          ...
          <span className="font-medium">{pagination.count}</span> companies
          {filteredCompanies.length !== companies.length && (
            <span className="ml-2">
              (<span className="font-medium">{companies.length}</span> total in
              database)
            </span>
          )}
        </div>
