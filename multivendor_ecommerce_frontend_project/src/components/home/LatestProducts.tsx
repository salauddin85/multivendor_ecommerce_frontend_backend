...
const LatestProducts: React.FC<{ products: LatestProduct[] }> = ({ products }) => {
  ...
  const getPrice = (product: LatestProduct): string => {
  ...
  return (
    <div className="w-full hidden md:block xl:max-w-[250px] mx-auto border rounded-md order-3 xl:order-0 xl:shrink-0 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between p-3 mb-2 bg-gray-100">
        <h2 className="font-bold text-gray-900 text-sm">LATEST PRODUCTS</h2>
      ...
      {/* Products List */}
      <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4 xl:grid-cols-1 transition-all duration-300 ease-in-out p-4">
        {currentPageProducts.map((product) => (
          <Link
            ...
            className="block"
          >
            <div className="flex gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300 hover:opacity-75 transition-opacity cursor-pointer">
              {/* Product Image */}
